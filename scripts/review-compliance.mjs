...
for(const width of [1440,390,320,768]) {
  ...
  await call("Page.navigate",{url:"http://localhost:3106/compliance"});
  ...
  results.push(await evaluate(`(()=>{const d=document.querySelectorAll('main details');d.forEach(e=>e.open=true);return {faqSections:d.length,expanded:[...d].every(e=>e.open)}})()`));
  results.push(await evaluate(`({documentRequests:[...document.querySelectorAll('main a')].filter(a=>a.textContent.includes('Request document')).length,publicDocuments:document.querySelectorAll('a[download],iframe,object').length})`));
  if(width===390){
...
await call("Page.navigate",{url:"http://localhost:3106"});
