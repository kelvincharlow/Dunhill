...
export function ProjectGrid({ category = "All projects", projects }: { category?: string; projects: Project[] }) {
  const portfolioProjects = projects.filter(project => project.image);
  const state = portfolioState(category, portfolioProjects);
  return <>
    ...
    <div id="project-results">
      {state.visible.length > 0 ? <div className={styles.grid}>{state.visible.map(project => <Link className={styles.card} href={`/projects/${project.slug}${state.suffix}`} key={project.slug}>
        <div className={styles.cardImage}><ProjectPhoto project={project} sizes="(max-width: 800px) calc(100vw - 44px), (max-width: 1472px) 46vw, 665px" /><span className={styles.arrow} aria-hidden="true"><ArrowIcon /></span></div>
