  ...
  return <>
    ...
    <div id="project-results">
      {state.visible.length > 0 ? <div className={styles.grid}>{state.visible.map(project => <Link className={styles.card} href={`/projects/${project.slug}${state.suffix}`} key={project.slug}>
        <div className={styles.cardImage}><Image src={`/images/profile/${project.image}`} alt={project.title} fill sizes="(max-width: 800px) calc(100vw - 44px), (max-width: 1472px) 46vw, 665px" /><span className={styles.arrow} aria-hidden="true">↗</span></div>
        <div className={styles.cardCaption}><h2>{project.title}</h2></div>
      </Link>)}</div> : <div className={styles.empty}><h2>More projects to come.</h2><p>There are no projects displayed in this category yet. Explore our current portfolio.</p><Link className="button" href="/projects" scroll={false}>View all projects <span aria-hidden="true">↗</span></Link></div>}
    </div>
  </>;
}
